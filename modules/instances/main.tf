resource "aws_eip" "pub_ip_fixed" {
  ...
  vpc = true
  tags = merge({"Name" = format("%s-%s-%s-eip-%s", var.name, var.env, var.ec2_suffix, count.index+1)}, var.tags)
}
...
resource "aws_instance" "server" {
  ...
  tags = merge( var.tags, {"Name" = format("%s-%s-%s-ec2-%s-%d", var.name, var.env, var.ec2_suffix, element(var.az_ids, count.index) ,count.index+1)})
  volume_tags = merge({"Name" = format("%s-%s-%s-root-%s-%d", var.name, var.env, var.ec2_suffix, element(var.az_ids, count.index), count.index+1)}, var.tags)
...
resource "aws_ebs_volume" "data" {
  ...
  tags              = merge({"Name" = format("%s-%s-%s-data-%s-%d", var.name, var.env, var.ec2_suffix, element(var.az_ids, count.index) ,count.index+1)}, var.tags)
}
