...
resource "aws_db_parameter_group" "pgroup" {
  ...
  name = format("%s-%s-rds-paramerter-group-%s", var.name, var.env, local.random_str)
  description = "Database parameter group for ${var.db_identifier} in ${var.env} environment"
  ...
  tags = merge(var.tags, {"Name" = format("%s-%s-rds-paramerter-group", var.name, var.env)})
...
resource "aws_db_option_group" "ogroup" {
  ...
  name                     = format("%s-%s-rds-option-group-%s", var.name, var.env, local.random_str)
  option_group_description = "Database option group for ${var.db_identifier} in ${var.env} environment"
...
resource "aws_db_instance" "db" {
  ...
  identifier        = format("%s-%s-database-%s", var.name, var.env, var.db_identifier)
  ...
  tags = merge(var.tags, {"Name" = format("%s-%s-db", var.db_identifier, var.env)})
